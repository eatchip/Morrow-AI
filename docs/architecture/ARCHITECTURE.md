...
本文档定义 Morrow 的**架构标准与组织方法**，不承担"展示完整目标目录"或"列举具体业务模块"的职责。

架构文档回答两件事：
- 代码应按什么规则组织。
- 不同层与不同 context 分别负责什么、禁止什么。

具体落地结构应以当前代码为准；首个代码 SDD 落地后，在 `DEVELOPMENT.md § 项目结构` 附一份"当前结构快照"。
...
- **`context` 是一级业务组织单位**。
- `app/main`、`app/preload`、`app/renderer` 只负责**进程边界与组合**，不是业务 owner。
- 不以"进程目录 + 杂糅业务逻辑"作为最终业务结构。
...
- 不允许依赖 `React / Electron / CLI / DB / FS / network / window.morrowApi`。
...
### `infrastructure`
- 实现 `application` 所需端口。
- 负责 `Electron / IPC / watcher / CLI / DB / FS / network` 对接。
- 不拥有业务规则，只负责技术接入。
...
- 会加载 **native addon**（Node-API/N-API），且其异常可能导致进程级 `abort / segfault`。
- 会执行不受信任或高复杂度的外部命令，并需要长时间保持运行。
...
外部可驱动能力（例如桌面 IPC、CLI、远程 API 等任何未来 transport）应收敛为一套 **Control Surface**（`command / query / event`）。

强制要求：

- 外部 client 禁止直读写持久化、直改 renderer state；必须通过 Control Surface 调用 `application/usecases`。
- IPC handler 只能做：`runtime validate -> mapping -> invoke usecase -> map result`；不得承载长流程业务编排。
- `Query` 必须无副作用；`Command` 的副作用必须可解释、可恢复（满足恢复模型与 owner 表要求）。
- 所有输入必须 **runtime validate**，输出必须走统一的结构化错误语义，避免把异常形态泄漏给调用方。
...
- `infrastructure`：contract / integration tests 验证 adapter、watcher、persistence。
- `recovery / hydration / resume / watcher`：必须至少有一条 integration 回归；关键用户路径保留 e2e。

详细跑法见 `DEVELOPMENT.md § 测试分层与 E2E 策略`。
...
- 在 renderer 业务流程文件里直接调用 `window.morrowApi`。
- 用持久化结构反向定义业务语义。
- 让非 owner context 判定别人的 durable truth。
- 通过跨 context 直接写 store / state 传递业务真相。
- 在 `main` 进程内直接承载高风险 native / 外部运行时，导致单点崩溃 / 白屏，缺失故障隔离与可恢复语义。
- 把 `application` 对外暴露为"大而全"的单一 API；应保持小而专用的端口。
